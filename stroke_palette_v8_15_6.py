...
MIN_COLOR_PROP = 0.002      # 0.2% 以下視為太小顏色（可調）
...
def main():
    ...
    ap.add_argument("--input", required=True, help="Path to an image file OR a folder containing images")
    ...
    if os.path.isdir(args.input):
        ...
        for p in paths:
            ...
            out_base = os.path.join(args.out_dir, stem)
            print(f"\n== {os.path.basename(p)} ==")
            process_one(p, out_base)
    else:
        stem = os.path.splitext(os.path.basename(args.input))[0]
        out_base = os.path.join(args.out_dir, stem)
        process_one(args.input, out_base)
